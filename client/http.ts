...
/**
 * A supervised adapter adds its own private instance-capability header here. The
 * capability is a transport credential for one control namespace; it never
 * reaches a result, an error or a rendered message.
 */
export interface ClientRequestOptions {
  instanceHeader?: string;
  instanceToken?: string;
  timeout?: number;
  /** Override transport-retry classification. Default: every mutation is ambiguous on transport failure. */
  ambiguous?: boolean;
}
...
export class ClientHTTP {
  ...
  async request(path: string, method = 'GET', body?: unknown, confirmation?: string, ifMatch?: string, idempotencyKey?: string, options: ClientRequestOptions = {}): Promise<any> {
    ...
    const mutation = options.ambiguous ?? (method !== 'GET' && path !== '/forge/v1/tools/issue_timeline');
    ...
    try {
      ...
      if (options.instanceHeader !== undefined && options.instanceToken !== undefined) headers[options.instanceHeader]=options.instanceToken;
      const text=body === undefined ? undefined : JSON.stringify(body);
      if (text && Buffer.byteLength(text)>1_048_576) throw new ForgeError('too_large','Request exceeds 1 MiB');
      const response=await this.#fetch(this.#url+path,{method,headers,body:text,redirect:'error',signal:AbortSignal.timeout(options.timeout ?? 15_000)});
