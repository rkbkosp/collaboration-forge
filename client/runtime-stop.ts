...
export function stopDecision(state:any,input:any,policy:'strict'|'advisory'='strict'):StopOutput{
 if(!state.active&&!state.pending&&!state.unknown)return {};
 const reason=state.pending?'Forge has an unresolved execution request. Run forge codex retry before claiming success.':`Forge still holds ${state.issue||'an issue'}. Record truthful evidence and close it if complete, or release it when handing off. Use forge codex pause before waiting for user input.`;
