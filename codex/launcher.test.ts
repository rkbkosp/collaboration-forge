...
test('managed worktrees use the same runtime CLI while ordinary paths keep host project routing',async()=>{
 ...
 const dir=await mkdtemp(join(tmpdir(),'forge-shim-'));const root=join(dir,'managed');const work=join(root,'trees','issue','tenure','repo','job','worktree');await mkdir(work,{recursive:true});
